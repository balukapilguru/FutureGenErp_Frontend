...
const UpcomingFeeRecordsList = () => {
  ...
  const [coursesOptions, setCoursesOptions] = useState(coursesList || []);
  useEffect(() => {
  ...
  const initialState = [
    ...
    { label: "TO Date", type: "date", inputname: "filter[toDate]", value: "", min: new Date(new Date().setDate(new Date().getDate() + 1)).toISOString().split('T')[0] },
    // {
    //   label: "Course",
    //   type: "select",
    //   inputname: "course",
  ...
  const exportDueFeesToExcel = async (endpoint, showLoader = true) => {
    ...
    try {
      ...
      const url = `${baseUrl}/fee/upcomingduefeerecords?page=${page}&pageSize=${UpcomingFeeRecordsList?.searchResultStudents}&search=${search}&filters[branch]=${branch}&filters[course]=${course}&filters[modeOfTraining]=${modeOfTraining}&filters[enquiryTakenby]=${enquiryTakenby}&filters[leadsource]=${leadsource}&filters[status]=${status}&filters[fromDate]=${fromDate}&filters[toDate]=${toDate}&filters[admissionFromDate]=${admissionFromDate}&filters[admissionToDate]=${admissionToDate}&filter[status]=${status}`;
      ...
      const records = data?.upcomingFeeRecords || [];
      ...
      const sheetName = `${branchLabel ? branchLabel + "_UpcomingDueFees" : "UpcomingDueFees"}`;
  ...
  const UpcomingFeeRecordsListData = UpcomingFeeRecordsList?.upcomingFeeRecords?.map((item) => ({
    ...item,
    rawItem: item,
  })) || [];
  ...
  const actions = (row) => {
    ...
    return (
      ...
      >
        <Link to={`/student/feeUpdate?studentId=${item?.studentDetails?.id}`}>
          ...
          <div className="card border-0">
            ...
            <div className="card-body">
              <div className="table-responsive table-container table-scroll table-card  border-0">
                <CustomTable
                  data={UpcomingFeeRecordsListData}
                  columns={columns}
                  actions={actions}
...
//                     to={`/student/feeUpdate?studentId=${item?.studentDetails?.id}`}
//                   >
