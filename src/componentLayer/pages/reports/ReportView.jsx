...
export const viewReportLoader = async ({ request, params }) => {
    const id = params?.id;
    if (!id) {
    ...
    try {
        ...
        const [reportByIdResult, reportsResult, studentsResult] = await Promise.allSettled([
            ERPApi.get(`/reports/getreport/${id}`),
            ERPApi.get(`/reports/getreports`),
            ERPApi.get(`/student/getstudent_data`),
        ]);

        // Helper to safely extract data and show toast on failure
        const getDataOrToast = (result, label, fallback = null) => {
        ...
        // Extract safe values
        const reportByIdData = getDataOrToast(reportByIdResult, "Report by ID", {});
        const allReportsData = getDataOrToast(reportsResult, "All Reports", []);
        const studentsData = getDataOrToast(studentsResult, "Students", []);

        // Optional: format reports with readable reportType
        // const reportsWithReportType = Array.isArray(allReportsData)
        //     ? allReportsData.map((report) => ({
        //         ...report,
        //         reportType: report.reports?.[0]?.reportType || "No Report Type Available",
        //     }))
        //     : [];

        // Optional: extract first report if needed
        const filteredReport = reportByIdData?.report?.reports?.[0] ?? null;


        // ✅ Return only 3 objects as you intended
        return {
            reportById: filteredReport,
            reportsData: allReportsData,
            students: studentsData,
        };
    } catch (error) {
        console.error("Error in viewReportLoader:", error);
        toast.error("Something went wrong while loading report data");
...
const ReportView = () => {
    ...
    useEffect(() => {
        if (reportsData) {
            const reportsWithReportType = reportsData?.map((report) => {
                return {
                    ...report,
                    reportType:
